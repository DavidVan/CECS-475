	...
	public class RookTests : ChessTests {
		...
		public void RookCapture() {
			ChessBoard b1 = CreateBoardWithPositions(
				 ...
				 Pos("b2"), ChessPieceType.RookKing, 1
				 );
			b1.GetPieceAtPosition(Pos(1, 1)).PieceType.Should().Be(ChessPieceType.Pawn, "Black Pawn");
			ApplyMove(b1, Move("b2", "b7"));
			b1.GetPieceAtPosition(Pos(1, 1)).PieceType.Should().Be(ChessPieceType.RookKing, "White Rook Capture");
		}
